...
export async function getOriginDefaultBranchName(
...
export async function getAllWorkdirs(repoUrl: string) {
  const tmp =
    process.env.SURF_ORIGINAL_TMPDIR ||
    process.env.TMPDIR ||
    process.env.TEMP ||
    "/tmp";
...
export async function getChangedFiles(
  ...
  const defaultRemoteBranch = await getOriginDefaultBranchName(
    targetDirname,
    token,
  );

  d(`Using origin/${defaultRemoteBranch} as remote default branch`);
  const remoteHeadCommit = await git(
    ["rev-parse", `origin/${defaultRemoteBranch}`],
    targetDirname,
  );
  ...
  return parseGitDiffOutput(
    await git(["diff", "--numstat", "origin/HEAD...HEAD"], targetDirname),
  );
}

export function getWorkdirForRepoUrl(
  repoUrl: string,
  sha: string,
  dontCreate = false,
) {
...
export function getTempdirForRepoUrl(
  repoUrl: string,
  sha: string,
  dontCreate = false,
) {
...
export async function pushGistRepoToMaster(
  targetDirname: string,
  token: string,
) {
