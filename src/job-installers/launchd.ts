...
const makeLaunchdService = compileTemplate(
  fs.readFileSync(path.join(__dirname, "launchd.plist.in"), "utf8"),
);
...
export default class LaunchdInstaller extends JobInstallerBase {
  ...
  async installJob(name: string, command: string, returnContent?: boolean) {
    ...
    let opts: Record<string, any> = {
      ...
      envs: this.getInterestingEnvVars().map((x) => [
        xmlescape(String(x)),
        xmlescape(String(process.env[x] || "")),
      ]),
      name,
    };
