...
const d = createDebug('surf:task-scheduler')
...
try {
  const runas = (await import('runas')).default
...
export default class TaskSchedulerInstaller extends JobInstallerBase {
  ...
  getPathToJobber() {
    const spawnRx = path.dirname(fileURLToPath(import.meta.resolve('spawn-rx/package.json')))
