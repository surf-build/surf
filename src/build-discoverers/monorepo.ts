...
export default class MonoRepoBuildDiscoverer extends BuildDiscoverBase {
  ...
  async getAffinityForRootDir() {
    ...
    try {
      ...
      this.surfConfiguration = JSON.parse(readFileSync(surfPath, 'utf8'))
    } catch (e) {
      d(`Failed to parse Monorepo JSON: ${e.message}`)
      return 0
    }

    if (!this.surfConfiguration?.projects || this.surfConfiguration.projects.length < 1) return 0
  ...
  async getBuildCommand(sha: string): Promise<BuildCommandResult> {
    ...
    try {
      return await asyncReduce(
        projectsToBuild,
        async (acc: BuildCommandResult, x: string) => {
          const thisProject = await determineBuildCommands(path.join(this.rootDir, x), sha)
