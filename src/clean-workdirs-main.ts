...
export default async function main(argv: any, showHelp: () => void) {
  ...
  const safeShas = refInfo.map((ref) => `-${ref.object.sha.substr(0, 6)}`)

  d(`safeShas: ${Array.from(safeShas).join()}`)
  const allDirs = await getAllWorkdirs(repo)
  const toDelete = allDirs.filter((x) => !safeShas.find((sha) => x.indexOf(sha) > 0))
  ...
  if (argv['dry-run']) {
    ...
    await asyncMap(toDelete, (x) => {
      ...
      return rimraf(x).catch((e) => console.error(`Tried to burn ${x} but failed: ${e.message}`))
    })
  }
}
